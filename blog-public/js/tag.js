...
function renderPage() {
  ...
  if (!page.length) { UI.emptyState(grid, I18n.t('tag.no_posts')); document.getElementById('pagination').innerHTML = ''; return; }
...
async function init() {
  ...
  if (!slug) { document.getElementById('page-title').textContent = I18n.t('tag.invalid'); return; }

  const grid = document.getElementById('posts-grid');
  UI.loading(grid, I18n.t('tag.loading'));
  ...
  try {
    ...
    if (tag?.imageUrl) {
      ...
      if (heroImg) { heroImg.src = tag.imageUrl; heroImg.alt = tag.title || slug; heroImg.style.display = 'block'; }
    }

    allPosts = Array.isArray(posts) ? posts : (posts?.content || []);
    renderPage();
  } catch (err) {
    UI.emptyState(grid, I18n.t('tag.cannot_load'), err.message);
  }
}
